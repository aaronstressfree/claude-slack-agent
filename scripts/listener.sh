...
while true; do
  ...
  if [ "${count:-0}" -gt 0 ] 2>/dev/null; then
    # Ack-once-per-batch guard: post :loading_: only when the highest message
    # ts in this batch is newer than the last one we acked. Without this,
    # every listener restart finds the same queued messages and re-acks,
    # producing thread noise. Stamp the file with the latest ts we saw.
    highest_ts=$(echo "$result" | python3 -c "import sys,json; msgs=json.load(sys.stdin).get('messages',[]); print(max((m.get('ts','0') for m in msgs), default='0'))" 2>/dev/null)
    last_acked=$(cat "$STATE_DIR/last_acked_ts" 2>/dev/null || echo "0")
    if [ "$(printf '%s\n%s\n' "$last_acked" "$highest_ts" | sort -g | tail -1)" = "$highest_ts" ] \
       && [ "$highest_ts" != "$last_acked" ] \
       && [ -n "$highest_ts" ] && [ "$highest_ts" != "0" ]; then
      python3 "$SCRIPTS_DIR/alert.py" post ":mag: Looking into it..." 2>>"$LOGFILE"
      echo "$highest_ts" > "$STATE_DIR/last_acked_ts"
    fi
    # Output to stdout, Claude sees this in the task notification.
    # Exec-respawn removed 2026-05-10: a nohup-respawned listener is detached
    # from the harness's task notification system, so its exit-with-messages
    # never reaches Claude. The AGENT is responsible for restarting the
    # listener via a fresh `Bash run_in_background:true` call after replying.
    echo "$result"
    exit 0
  fi
  sleep 6
